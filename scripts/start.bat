...
if not exist "run.py" (
    echo [ERROR] run.py not found. Run this script from project root.
    exit /b 1
)
...
set "SERVER_ARGS=server --port %PORT%"
if /I "%MODE%"=="prod" set "SERVER_ARGS=server --waitress --port %PORT%"
if /I "%MODE%"=="dev" set "SERVER_ARGS=server --dev --port %PORT%"
...
if /I "%RUNNER%"=="uv" (
    uv run python run.py %SERVER_ARGS%
    set "EXIT_CODE=!ERRORLEVEL!"
    if not "!EXIT_CODE!"=="0" (
        if "%HAS_PY%"=="1" (
            echo [WARN ] uv launch failed. Trying python directly...
            python run.py %SERVER_ARGS%
            set "EXIT_CODE=!ERRORLEVEL!"
        )
    )
) else (
