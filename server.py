...
def recharge():
    ...
    if amount not in recharge_tiers:
        return (
            jsonify({"error": "无效的充值金额，只支持 6、30、98、198、328、648 元"}),
            400,
        )
    ...
    if from_resource == "origeometry":
        if to_resource == "oroberyl":
            ...
            if user_info["resources"].get("origeometry", 0) >= amount:
                ...
                # 更新最后访问时间
                user_info["last_visit"] = (
                    __import__("datetime").datetime.now().isoformat()
                )
                # 保存用户数据
                save_user(user_id, user_info)
                return jsonify(
                    {"message": f"成功兑换 {amount} 衍质源石为 {amount * 75} 嵌晶玉"}
                )
            else:
            ...
            if user_info["resources"].get("origeometry", 0) >= amount:
                ...
                save_user(user_id, user_info)
                return jsonify(
                    {"message": f"成功兑换 {amount} 衍质源石为 {amount * 25} 武库配额"}
                )
            else:
                return jsonify({"error": "衍质源石不足"}), 400
        else:
