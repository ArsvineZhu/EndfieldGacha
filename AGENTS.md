...
- **`Scheduler`**：策略调度器（支持多卡池连续模拟、批量并行评估）
- **预定义魔数**：`URGENT`（加急招募）、`DOSSIER`（情报书）、`UP_OPRT`（UP角色）、`OPRT`（6星角色）
...
| **策略调度模块** | `scheduler.py` | 魔数编码策略系统和科学评分系统 |
| **演示工具模块** | `demo.py` | 抽卡演示与统计分析工具 |
| **策略评估脚本** | `evaluation.py` | 预置抽卡策略评估脚本 |
| **概率验证工具** | `examination.py` | 概率分布验证工具 |
...
---
*文档版本：2.2.0 | 最后更新：2026-03-06 | 适用AI助手：opencode等*
